...
angular.module("myApp").controller("designCntl", function ($scope, $location, state, defaults, persist) {
    ...
    function getDesignDocs(db) {
        console.log('initscreen #design');
        
        $scope.designDocs = {};
        couchapi.docAllDesign(db).when(
                function(data) {
                    console.log(data);
                    var ddocPromises = [];
    ...
    function defineDbGrid() {
        ...
        $scope.dbColumnDefs =
            [
                {visGroup:'value', field:'name', displayName:'Select database(s)', enableCellEdit: false, visible:true, width:130}
            ];
